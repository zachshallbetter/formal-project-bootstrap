...
Checks, in order and stopping at the first failure:
  ...
  notice      GET /notice?owner=&name=            governance state of this repository (informational)
...
def settings() -> dict:
    ...
    repo_name = env.get("ACP_REPOSITORY_NAME") or git_repo_name()
    return {
...
def main() -> int:
    ...
    try:
        ...
        if cfg["owner"] and cfg["repo_name"]:
            status, body = call(
                cfg["url"],
                "/notice?" + urlencode({"owner": cfg["owner"], "name": cfg["repo_name"]}),
                None,
                args.timeout,
            )
            summary["checks"]["notice"] = "OK" if status == 200 else f"HTTP {status}"
            ...
            summary["checks"]["notice"] = "SKIPPED (owner or repository name unknown)"
